...
def build_data(sentence_list, stopwords):
    # a list of sentences
    ...
    output_save_path = os.path.join('ETM', 'data', 'tweets', 'min_df' + str(min_df))+'/'
    if not os.path.exists(output_save_path):
        os.mkdir(output_save_path)

    with open(output_save_path + 'vocab.pkl', 'wb') as f:
        pickle.dump(vocab, f)
...
def read_data(data_path='data/dedup_en_tweets_dest_1-2.csv'):
...
if __name__ == '__main__':
    ...
    sents = []
    for text in texts:
        sentence = preprocessing_text(text)
        # print(sentence)
        sents.append(sentence)

    sents= shuffle(sents)
    stopwords = load_stopwords()
    build_data(sents, stopwords)
    # print(word2id)
