...
parser.add_argument('--data_file', type=str, default='datasets/tweets_df_tm_processed.csv', help='a .txt file containing the corpus')
parser.add_argument('--emb_file', type=str, default='preprocessed_data/embeddings.txt', help='file to save the word embeddings')
...
def read_data(data_path=args.data_file):
    df = pd.read_csv(data_path)
    texts = [x.split() for x in df.preprocessed_text.tolist()]
    return texts


sentences = read_data()  # a memory-friendly iterator
print('sentences:',len(sentences), sentences[:1])
...
model.save('preprocessed_data/embeddings.model')